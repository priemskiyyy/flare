...
| Sentry composition        | The installed browser SDK runs in Node through `beforeSend`, verifying identity and breadcrumbs on a buffered report after an account switch. `pnpm test:providers` sends its envelopes over its own fetch transport to a loopback receiver: the event on the wire carries the report under the event id the receipt names, and `flush` returns once the server answered. | Browser integrations, sampling, symbolication, native scope behavior, Sentry's own ingestion. |
| PostHog composition       | The installed posthog-js runs in jsdom with `fetch` stubbed, and `before_send` reads each event: the merge over the person and the super properties, the exception steps, and a dropped event. `pnpm test:providers` lets it send over its own transport to a loopback receiver, and reads the `$exception` event and its uuid off the wire.                              | Batching, compression, the React Native client, PostHog's own ingestion.                      |
| OpenTelemetry composition | The installed `@opentelemetry/sdk-logs` runs with an in-memory exporter, checking that every attribute survives the SDK's validation, with the record's severity, body and capture time. `pnpm test:services` exports over OTLP HTTP to a real OpenTelemetry Collector in Docker and reads back what it wrote, nested attributes included.                                | The protobuf and gRPC exporters, a batching processor, a backend behind the collector.        |
| Bugsnag delivery          | `pnpm test:providers` runs the browser notifier of `@bugsnag/js` in jsdom against a loopback receiver: a report is `submitted` only once the server answered its delivery, and a refused delivery is `failed`.                                                                                                                                                            | The React Native notifier, sessions, Bugsnag's own ingestion.                                 |
...
| HTTP adapter              | Unit tests over an in-process fake `request`, standing for the application's own client. `pnpm test:providers` runs it over `fetch` against a loopback server: the body and idempotency key on the wire, a 503, and a deadline that abandons the request.                                                                                                                 | A production server, retries in a real client.                                                |
...
| Devtools                  | A real shadow root in jsdom, queried by role, and each wrapper rendered through its own binding. The examples' browser tests open it in Chromium through the React, Vue, Solid and Svelte wrappers and check that it lists the destinations.                                                                                                                              | Resizing and pointer dragging in a real browser.                                              |
| Example applications      | Unit tests in jsdom drive the React, Vue, Solid and Svelte examples through their buttons, and the fixture server over real HTTP. `pnpm test:examples` drives the four built pages in Chromium with Playwright at 375 and 1280 px. `pnpm check:release` bundles the Expo app for web, iOS and Android.                                                                    | Other browsers, every real provider SDK: the pages simulate them. The Expo app on a device.   |
...
- No adapter has been run against its provider's real backend from this repository. The Sentry, PostHog, Bugsnag and HTTP transports have only reached loopback receivers that accept whatever arrives, and OpenTelemetry records only a local collector.
- Nothing has run on an iOS or Android device, in a simulator or in Hermes. The Expo example is bundled for iOS and Android, and its web export was driven in Chromium against the fixture server once, by hand.
- Only the example applications have run in a real browser, in Chromium, and over simulated provider SDKs. The devtools' resizing and pointer dragging have not been exercised in a browser.
...
This process is not part of continuous integration. Continuous integration runs the type checks, the linters, the unit tests on two Node and React versions and on one version each of Vue, Solid and Svelte, the provider transport tests, the collector tests in their own workflow, the packed-consumer check and the documentation checks. The examples' browser tests are not part of it.
...
The first command builds the packages and runs formatting, types, lint, the unit tests and the provider transport tests. The second adds the documentation checks, the packed-consumer check and the release metadata check. The third builds the packages and runs the examples' browser tests in Chromium, through `@playwright/test`. The fourth needs Docker: it starts an OpenTelemetry Collector, runs the service tests against it and stops it again.
