...
- No adapter has been run against its provider's real backend from this repository. The Sentry, PostHog, Bugsnag and HTTP transports have only reached loopback receivers that accept whatever arrives, and no log record was sent to an OpenTelemetry collector.
...
This process is not part of continuous integration. Continuous integration runs the type checks, the linters, the unit tests on two Node and React versions and on one version each of Vue, Solid and Svelte, the provider transport tests, the packed-consumer check and the documentation checks. The examples' browser tests are not part of it.
