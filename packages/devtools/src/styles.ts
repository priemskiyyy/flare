...
:host {
  ...
  --accent: #ffb454; --accent-text: #ffd9a3; --ok: #3fd99a; --warn: #f0b653; --error: #ff7d8c; --info: #5ad8ff; --well: #090c11;
...
@media (prefers-color-scheme: light) {
  :host {
    ...
    --accent: #b45309; --accent-text: #92400e; --ok: #15803d; --warn: #b45309; --error: #dc2626; --info: #0a84c7; --well: #f3f5f9; color-scheme: light;
  }
}
...
.destinations { display: block; overflow: auto; padding: 8px; border-right: 1px solid var(--border); }
.destination { display: flex; align-items: center; flex-wrap: wrap; gap: 6px; width: 100%; margin: 0 0 2px; padding: 7px 10px; border-color: transparent; background: transparent; text-align: left; }
.destination[aria-pressed="true"] { background: color-mix(in srgb, var(--accent) 12%, transparent); border-color: color-mix(in srgb, var(--accent) 30%, transparent); color: var(--accent-text); }
.destination-name { display: flex; align-items: center; gap: 9px; overflow-wrap: anywhere; font: var(--mono); }
.destination small { width: 100%; color: var(--muted); font-size: 10px; }
...
.chip[data-kind="REPORT"]::before { background: var(--accent); }
.chip[data-kind="DESTINATION"]::before { background: var(--ok); }
.chip[data-kind="SESSION"]::before { background: var(--info); }
...
.event summary { display: grid; grid-template-columns: 84px 170px 100px 70px minmax(0, 1fr) 10px; gap: 12px; align-items: center; padding: 7px 14px; cursor: pointer; list-style: none; font: var(--mono); }
...
.event-destination, .event-report, .event-summary { color: var(--muted); }
.event[data-kind="REPORT"] { box-shadow: inset 2px 0 0 var(--accent); }
.event[data-kind="REPORT"] .event-type { font-weight: 500; }
.event[data-kind="DESTINATION"] .event-type { color: var(--ok); }
.event[data-kind="SESSION"] .event-type { color: var(--info); }
