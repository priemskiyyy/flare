...
/**
 * Sends exceptions to PostHog error tracking through the posthog-react-native
 * client the application created. Each report is one `$exception` event
 * whose properties are its own. The adapter waits until the client has
 * loaded its storage, then sends a report only while the person the client
 * identifies is the report's user, and flushes the client's queue. Messages
 * are skipped: PostHog tracks exceptions.
 *
 * @example
 * ```ts
 ...
 * const flare = new Flare({ destinations: { posthog: posthog({ sdk: client }) } });
 * ```
 */
export const posthog = <TSdk extends PostHogLike>({
  ...
  open: () => ({
    native: sdk,
    submit: async (report, { signal }) => {
      if (report.kind === "message") {
        return { status: "skipped", reason: "unsupported-report-kind" };
      }

      // Until then the distinct id is empty, and would match no user.
      await sdk.ready();

      // The runtime settled the report while the client was loading.
      if (signal.aborted) {
      ...
      if (user !== null && user.id !== sdk.getDistinctId()) {
