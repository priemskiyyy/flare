...
Send [Flare](../../core) error reports to Bugsnag, in the browser and in React Native. You pass in the Bugsnag SDK your application already started, so this package imports none, and a browser bundle can never pull in a React Native SDK.
...
On React Native, import `Bugsnag` and `Breadcrumb` from `@bugsnag/react-native` instead. The adapter and everything else are the same:
...
| ------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `sdk`        | required | The static Bugsnag API your application started: the default export of `@bugsnag/js` or `@bugsnag/react-native`.                            |
| `Breadcrumb` | required | Bugsnag's `Breadcrumb` class, a named export of the same package. A report's breadcrumbs are written onto its own event as instances of it. |
...
| report id         | `report_id` in the `flare` metadata section, so one report can be found in every destination        |
...
- Starting is yours. Flare never starts Bugsnag, and Bugsnag cannot be stopped. `Bugsnag.notify` only logs when Bugsnag has not been started, and never calls back, so an SDK that is not started yet fails the destination's start with a `FlareError` whose code is `NOT_INITIALIZED`. Call `flare.start()` again after `Bugsnag.start` and it succeeds, and the reports captured meanwhile are delivered while they are still in Flare's buffer.
- Evidence is `sdk-callback-completed`. The post-report callback fires without an error for an event that was delivered, for one queued for later delivery, and also for one that an `onError` callback of your own discarded or that your release stage disabled. It cannot tell these apart, and neither can Flare. A callback with an error is a `failed` outcome. There is no event id.
...
- Queue and offline: the browser notifier delivers at once and keeps nothing for later. On React Native the native layer stores events on the device and sends them later.
- There is no `flush` in either SDK. `flare.flush()` drains this destination and reports its boundary as `unsupported`.
- Bugsnag's static API is one process-wide instance. Register it under one destination name: two destinations over the same SDK report every event twice.
- `native` is the SDK you passed in, fully typed, for everything Flare does not wrap, such as sessions or feature flags. Calls made on it bypass Flare's routing, receipts and privacy.
...
- Reports Flare submits never inherit what the mirror wrote. Bugsnag copies the client onto every event, and the mirrored state describes the current account, so the adapter clears those sections from each event before it writes the report's own. A report captured under one account and delivered after a switch therefore carries nothing of the next account.
- Bugsnag has no way to remove a breadcrumb from the client. Mirrored breadcrumbs are stamped under the key `flare.generation`. Each submitted event removes these mirrored entries and restores its captured history using the `Breadcrumb` class, while retaining Bugsnag's own entries.
