...
import type { FlareProviderProps } from "../types/FlareProviderProps.js";
import type {
  RegisteredDestinationName,
  RegisteredDestinations,
  RegisteredSchema,
} from "../types/Register.js";
import { useDestinationStatus } from "./useDestinationStatus.js";
import { useFlare } from "./useFlare.js";
import { useFlareStatus } from "./useFlareStatus.js";
...
export const useTypeContracts = () => {
  ...
  expectTypeOf(useFlare().current).toEqualTypeOf<
    Flare<Destinations, FlareSchema>
  >();
  expectTypeOf(useFlareStatus().current).toEqualTypeOf<FlareStatus>();
  expectTypeOf(
    useDestinationStatus("anything").current,
  ...
  status.current = { state: "started" };

  // @ts-expect-error -- a destination is named by a string.
  useDestinationStatus(7);
};

// Every boundary prop is optional, and may be forwarded as undefined.
expectTypeOf<Record<string, never>>().toExtend<FlareErrorBoundaryProps>();
expectTypeOf<{ capture: undefined }>().toExtend<FlareErrorBoundaryProps>();
